...
d = {}
d[temp[1].strip('"')] = temp[0].strip('"')
# print(d)

target = {}
with open(TICPATH) as file:
    for line in file:
        line = line.strip()
        if line:
            line = line.lower().split('-')
...
def read_dat(tic):
    ...
    path = os.path.join(data, f'{tic}_prc.dat')
    l = []
...
# line = '00000000000000 2005-09-27018.19491004943847719.299999221.0490021.399999618530273'
    ...
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            ...
            for i in tickers_lst:
                if i is not tic_exchange_dic.keys():
                    raise Exception
...
for i in t:
    ...
    for lines in lines:
        temp = get_tics(line)
        temp = {j:temp[j] for j in temp.keys()if j in l}
        print(temp)
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for i in tickers_lst:
        ...
        for line in lines:
            temp = get_tics(line)
            temp = {j: temp[j] for j in temp.keys() if j in col_lst}

            result[j]['data'].append(temp)
